...
void enemy::constant_walk(int newrate,int newhoming,int special)
{
	...
	if(clk3<=0)
	{
		...
		if(step==0)
			...
			clk3=int(16.0/((step==0)?((double)0.0f):step));
	}
	else if(scored)
	{
		dir^=1;
		clk3=int(16.0/((step==0)?((double)0.0f):step))-clk3;
	}
...
void enemy::constant_walk_8(int newrate,int newhoming,int special)
{
	...
	if(clk3<=0)
	{
		...
		clk3=int(8.0/((step==0)?((double)0.0f):step));
	}
...
eFire::eFire(zfix X,zfix Y,int Id,int Clk) : enemy(X,Y,Id,Clk)
{
	...
	if(flags & guy_fadeflicker)
	{
		...
		if(!canmove(down,(zfix)8,spw_none))
			clk3=int(13.0/((step==0)?((double)0.0f):step));
	}
	...
	if(clk3<=0)
	{
		...
		if(clk2<=0)
		{
			...
			if(!canmove(dir,(zfix)2,spw_none) || m_walkflag(x,y,spw_none, dir) || (rand()&15)>=hrate)
				clk2=(wpn==ewBrang ? 1 : 16*jump_width/((step==0)?((double)0.0f):step));
		}
		
		if(dmisc9!=e9tPOLSVOICE && dir>=left) //if we're moving left or right
		{
			clk2=16*jump_width/((step==0)?((double)0.0f):step);
		}
...
bool esMoldorm::animate(int index)
{
	...
	if(clk>=0)
	{
		...
		parentclk=(parentclk+1)%((int)(8.0/((step==0)?((double)0.0f):step)));
...
esPatra::esPatra(zfix X,zfix Y,int Id,int Clk) : enemy(X,Y,Id,Clk)
{
	//cs=8;
	item_set=0;
	misc=clk;
...
esPatraBS::esPatraBS(zfix X,zfix Y,int Id,int Clk) : enemy(X,Y,Id,Clk)
{
	//cs=csBOSS;
	item_set=0;
	misc=clk;
